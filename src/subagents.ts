...
export const mcpConfigPolicies = [
  "inherit_codex",
  "isolated",
  "explicit",
  "inherit_claude_project",
] as const;
export type McpConfigPolicy = (typeof mcpConfigPolicies)[number];
...
async function prepareTempCodexHome(
  definitions: CodexSubagentDefinition[],
  env: NodeJS.ProcessEnv,
  options: { isolated?: boolean; mcpServers?: Record<string, unknown> } = {},
): Promise<string> {
...
function serializeGeneratedConfig(mcpServers?: Record<string, unknown>): string {
...
async function readClaudeProjectMcpServers(projectDir: string | undefined): Promise<Record<string, unknown> | undefined> {
  ...
  for (const candidate of candidates) {
    try {
      ...
      // Try the next known Claude project MCP config location.
    }
  }
  return undefined;
}
