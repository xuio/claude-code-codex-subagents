...
- Durable session state and rotated log files are written with owner-only permissions.
...
Verbose stderr logging is enabled by default for debugging Claude Code behavior. These logs intentionally include raw MCP JSON-RPC frames, tool arguments/results, prompt text, and Codex stdin/stdout/stderr traffic. Treat local logs and exported debug bundles as sensitive project data; set `CODEX_SUBAGENTS_LOG_LEVEL=silent` to disable logging.
...
See [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md) for safer diagnostics and debug-bundle guidance.
